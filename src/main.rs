use card::{load_cards_from_file, parse_carddefs_xml, save_cards_to_file};
...
fn main() {
    ...
    info!("finished parsing {} cards from `hsdata/CardDefs.xml` in {:.2?}", defs.cards.len(), parse_duration);
    ...
    info!("read {} cards in {:.2?}", loaded_defs.cards.len(), load_duration);
